...
module Tree = {
  ...
  let set_max_right =
    ...
    | (Node(_, infoL, _), info, Node(_, infoR, _)) =>
      info.max_right =
        max(info.right, max(infoL.max_right, infoR.max_right));
  ...
  // 2. v is the tightest with this property - it is the smallest interval (each interval in the t should be either disjoint)
